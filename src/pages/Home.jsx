...
import { GAMES, SPORTS } from "@/lib/mock-data";
...
function Home() {
  ...
  const games = GAMES.filter((g) => {
    ...
    return g.title.toLowerCase().includes(q) || g.location.toLowerCase().includes(q) || g.sport.toLowerCase().includes(q);
  });
  ...
  />
    ...
    return <motion.button
      ...
      className={cn(
        ...
        active ? "border-primary bg-primary text-primary-foreground green-glow" : "border-white/10 bg-white/5 text-muted-foreground hover:bg-white/10 hover:text-foreground"
      )}
    >
                  {name}
                </motion.button>;
        ...
        <section className="mt-8">
          ...
          {games.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 py-20 text-center text-muted-foreground">
