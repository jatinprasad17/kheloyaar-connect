...
function Profile() {
  ...
  const [interests, setInterests] = useState(["Football", "Badminton", "Basketball"]);
  ...
  const toggle = (s) => setInterests((prev) => prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]);
  ...
  >
    ...
    return <button
      ...
      className={"rounded-2xl border px-4 py-2 text-sm font-medium transition-colors " + (active ? "border-primary bg-primary text-primary-foreground" : "border-white/10 bg-white/5 text-muted-foreground hover:bg-white/10 hover:text-foreground")}
    >
                  {s.name}
                </button>;
  })}
        ...
        <section className="mt-8">
          ...
          {created.length === 0 ? <p className="mt-3 rounded-2xl border border-dashed border-white/10 py-8 text-center text-sm text-muted-foreground">
              You haven't hosted a game yet.
            </p> : <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {created.map((g, i) => <GameCard key={g.id} game={g} index={i} />)}
            </div>}
        </section>

        <section className="mt-10">
          <h2 className="text-lg font-semibold">Games you joined</h2>
          {joined.length === 0 ? <p className="mt-3 rounded-2xl border border-dashed border-white/10 py-8 text-center text-sm text-muted-foreground">
              No joined games yet — explore the home page.
            </p> : <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {joined.map((g, i) => <GameCard key={g.id} game={g} index={i} />)}
            </div>}
        </section>
...
function Stat({ label, value }) {
  return <div className="rounded-2xl border border-white/10 bg-background/60 p-5">
