...
import { INITIAL_MESSAGES, CURRENT_USER, PLAYERS, type ChatMessage } from "@/lib/mock-data";
      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-6 py-5">
        <AnimatePresence initial={false}>
          {messages.map((m) => {
            ...
            const sender =
              PLAYERS.find((p) => p.id === m.senderId) ?? CURRENT_USER;
            return (
              <motion.div
                key={m.id}
              ...
              >
                {!mine && (
                  <img src={sender.avatar} alt={sender.name} className="h-7 w-7 rounded-full" />
                )}
                <div
                  className={cn(
                ...
                >
                  {!mine && (
                    <p className="mb-0.5 text-[11px] font-semibold text-primary">{sender.name}</p>
                  )}
